...
for iocn in range(nocn):
  ...
  if (cc % 20000) == 0:
    ...
    print(' {0:5.2f}% done {1:6.2f} min, tot {2:6.2f} min ...'.\
            format(cc/nocn*100,(toc-tic)/60,(toc-ticR)/60))
...
# Integrate N(z)
  lmbdW = eig_wkb(kbtm,N2z,ZZ,fcor,mode=1)
...
# Add 0 at the ends for eig/functions:
  ...
  plt.plot(vvV,zzV,'.-')
  ctl = 'Eig/vector lmbd=1 im={0}'.format(im)
  plt.title(ctl)
